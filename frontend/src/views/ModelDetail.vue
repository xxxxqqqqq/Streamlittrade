...
async function openSealed(){
  changing.value=true;error.value=''
  try{
    const response=await api.post(`/models/${model.value.id}/sealed-evaluation`,{reason:reason.value})
...
<template>
  <section class="workflow">
    ...
    <template v-else>
      <article class="model-hero"><div class="feature-icon purple-bg"><Boxes :size="25"/></div><div><span class="eyebrow dark">REGISTERED MODEL</span><h2>{{model.name}}</h2><p>{{model.algorithm}} · Version {{model.version}}</p></div><div class="row-actions"><button v-if="model.prediction_artifact_uri" class="primary" @click="router.push(`/models/${model.id}/trade-workbench`)">模型交易工作台</button><button v-if="model.prediction_artifact_uri" class="secondary" @click="router.push({path:'/backtests/new',query:{model_id:model.id}})">用此模型创建回测</button></div><i class="status" :class="model.stage">{{model.stage}}</i></article>
