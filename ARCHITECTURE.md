...
- 导航收敛为顶部导航栏：左侧是品牌与“获取数据 → 因子 → 训练 → 回测 → 模拟盘”
  五段研究流一级入口，当前段用 pill 高亮；内容区全宽居中（最大 1400px）；
- 侧栏已删除，段内次级入口改为各段页面顶部的页签（`SectionTabs.vue`）：
  数据段=数据同步与版本 / 因子快照，因子段=因子研究，
  训练段=训练实验 / 研究数据集 / 模型仓库 / 模型比较，
  回测段=回测中心 / 策略版本 / 批量预测，模拟盘段=账户总览 / 自动调仓（`?tab=automation`）；
- 顶栏右侧固定放全局搜索、任务中心（有排队或运行中任务时显示角标）、通知铃铛和用户菜单；
  当前项目切换、平台治理分组（项目与成员 / 用户管理 / 审计日志 / 生产运行，仅 admin 可见）
  与退出登录统一收进用户菜单；
- 首页是状态驱动的工作台：顶部五段流状态条给出每段最近产物（名称、状态徽标、时间，
  可直达），再按“有无数据版本、快照、数据集、模型、回测”给出唯一的下一步建议，
  并汇总最近产物、任务状态与数据新鲜度；
...
- 前端已下线 `/quick-research` 入口和对应页面路由（`QuickResearch.vue` 文件保留，
  不再挂载）：一级导航改为顶栏五段研究流，段内视图用页签承载；首页改成状态驱动的工作台，
  由它给出唯一的下一步建议。流水线 API 与编排逻辑不变，仍可用分步页面或直接调用
