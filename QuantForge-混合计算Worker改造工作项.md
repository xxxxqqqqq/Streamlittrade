...
- 云端真实 38.26 万行、200 股票、87 因子完整计算两次，耗时 292 秒和 234 秒，产物哈希一致，峰值约 303 MiB；
- 新旧 12/36 因子快照逐行逐因子回归：日期/股票键一致，0 个数值不匹配，最大绝对误差 0；
- 基于新 87 因子快照完成 20 日因子研究，87 个因子全部参与，选出 20 个因子，峰值约 998 MiB；
- 笔记本已切换到专用 PostgreSQL 与限桶 MinIO 凭据，并以 38.26 万行单因子任务验证读写权限；
- 已完成 Worker 离线演练：重任务保持在 `quant-heavy` 排队、云 Worker 不抢任务，笔记本恢复后自动领取并成功完成；
- 后端 87 项测试全部通过。

云端兼容发布、数据库备份、回滚锚点、回环网关、SSH 隧道和 `split` 路由均已上线。剩余生产化观察项：

- 补充断网/中途强制重启专项演练、长期运行观察，以及 Redis ACL 进一步收紧（当前 Redis 只经 SSH 隧道访问）；
- 稳定后将相同镜像和外置配置移植到台式机。
